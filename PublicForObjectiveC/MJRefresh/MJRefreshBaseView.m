...
- (void)initial
{
    ...
    self.backgroundColor = [UIColor colorWithRed:242/255.0 green:243/255.0 blue:247/255.0 alpha:1];
...
//    [self addSubview:_activityView = activityView];
    ...
    imageView.bounds = CGRectMake(0, 0, 134, 64);
    imageView.center = self.center;
    NSMutableArray *animationArr = [[NSMutableArray alloc]init];
    for (NSInteger i =0; i <10; i++) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"refresh_animation_%02ld",i +1]];
        [animationArr addObject:image];
    }
    imageView.animationDuration = 3;
    imageView.animationImages = animationArr;
    imageView.animationRepeatCount = 0;
    [self addSubview:_animationImage =imageView];
...
- (void)setFrame:(CGRect)frame
{
    ...
    _animationImage.bounds = CGRectMake(0, 0, 134, 64);
    CGPoint center = CGPointMake(statusWidth/2, lastUpdateY);
    _animationImage.center = center;
}
